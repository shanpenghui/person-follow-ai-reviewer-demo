...
这是一个面试展示用项目：以 `D:\midea\person_follow` 的机器人跟随模块为样例，演示“AI 助力研发”如何落到 PR 评审、风险识别和测试补全。
...
> 我做了一个 AI PR Review 助手，接入机器人跟随模块的研发流程。它读取 PR diff，定位变更函数，结合项目风险规则检索上下文，再让 LLM 生成 review 建议和测试清单，帮助 reviewer 提前发现运动安全、深度异常、手势 FSM 和 Action 契约问题。
